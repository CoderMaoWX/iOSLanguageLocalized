...
+ (void)mappingLanguage:(NSString *)csvURL
    ...
    if (appLprojDict.allKeys.count == 0) {
        if (compeletion) {
            compeletion(NO, @"目录文件夹不存在需要翻译的多语言文件", NO);
        }
        return;
    }
...
+ (void)testMatchLanguage {
    ...
    "\"Register_ZFLanguageLocalized.com\" = \"To complete registration, you must agree to the ZFLanguageLocalized website Terms and Conditions.\";\n"
